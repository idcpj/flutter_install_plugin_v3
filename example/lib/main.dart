...
class _MyAppState extends State<MyApp> {
  /// 默认使用了再惠合伙人的下载地址
  static const _defaultUrl = 'https://itunes.apple.com/cn/app/%E5%86%8D%E6%83%A0%E5%90%88%E4%BC%99%E4%BA%BA/id1375433239?l=zh&ls=1&mt=8';
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          Text('downloading ${(_progressValue * 100).toStringAsFixed(0)} %')
                        ],
                      ),
                    ),
                    ElevatedButton(onPressed: () => _networkInstallApk(), child: Text('network install apk')),
                    SizedBox(height: 10),
                    ElevatedButton(onPressed: () => _localInstallApk(), child: Text('local install apk')),
                  ],
                )
              : Column(
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: <Widget>[
                    Platform.isIOS
                        ...
                        : SizedBox(),
                    Platform.isIOS ? ElevatedButton(onPressed: () => _gotoAppStore(_appUrl), child: Text('gotoAppStore')) : SizedBox()
                  ],
                ),
        ),
      ),
  ...
  _gotoAppStore(String url) async {
    ...
    _showResMsg("go to appstroe ${res['isSuccess'] == true ? 'success' : 'fail:${res['errorMessage'] ?? ''}'}");
  }
